...
<div class="content">
    <div class="row">
        ...
        ?>
            <div class="col-sm-8 col-9 text-right m-b-20">
                <a href="<?=base_url('ngo/requestOrder');?>" class="btn btn btn-primary btn-rounded float-right"><i class="fa fa-plus"></i> Request New Order</a>
            </div>
        <?php
            }
        ?>
    ...
    <form action="<?= base_url('panel/requestOrder');?>" method="post">
        <?php
            if(isset($current_order_id)){
        ?>
                <div class="row">
                    <div class="col-sm-12">
                        <a href="<?= base_url('panel/requestOrder');?>" style="color:red;">Clear Search</a>
                    </div>
                </div>
        <?php
            }
                            ...
                            <td class="text-right">
                                <div class="dropdown dropdown-action">
                                    ...
                                    <div class="dropdown-menu dropdown-menu-right">
                                        <a class="dropdown-item" href="<?= base_url('panel/viewRequestOrder').'/'.$rec->id;?>"><i class="fa fa-pencil m-r-5"></i> View Order</a>

                                    <?php
                                        if($rec->status == 'Pending' && $current_role == 'restaurant'){
                                    ?>
                                            <a class="dropdown-item" href="<?= base_url('restaurant/requestOrder').'/'.$rec->id;?>"><i class="fa fa-pencil m-r-5"></i> Edit</a>
                                    <?php
                                        }

                                        if(in_array($rec->status, ['Pending', 'Accepted']) && empty($rec->rider_id) && $current_role == 'ngo'){
                                    ?>
                                            <a class="dropdown-item" href="<?= base_url('ngo/editRequestOrder').'/'.$rec->id;?>"><i class="fa fa-pencil m-r-5"></i> Edit</a>
                                    <?php
                                        }
                                    ?>
                                    </div>
